...
makeSuite('LendingPool liquidation - liquidator receiving aToken', (testEnv) => {
  ...
  // NOTE: Disabling tests for aToken liquidation as currently liquidation is restricted to fundlock
  // and it does not support receiving aTokens.
  ...
  //   await expect(
  //     pool.liquidationCall(weth.address, dai.address, borrower.address, 1, true, 0)
  ...
  //   await expect(
  //     pool.liquidationCall(dai.address, dai.address, borrower.address, oneEther.toString(), true, 0)
